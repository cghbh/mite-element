<template>
	<div class="m-collapse-item">
		<!-- 折叠面板的头部信息展示 -->
		<div class="m-collapse-item-head" @click="handleClick">
			<span class="m-collapse-item-head-text">{{ title }}</span>
			<span class="m-collapse-item-head-icon icon-style" :class="{'is-active': isActive, 'icon-back': isActive}">
		...
		<!-- 折叠面板的主体信息展示 -->
		<collapse-transition>
			<div class="m-collapse-item-content" v-if="isActive">
				<slot></slot>
			</div>
		</collapse-transition>
	</div>
